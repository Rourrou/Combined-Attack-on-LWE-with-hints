...
def sol_perfect_hints(m, k, solution):
    ...
    with open("Data/Perfect Hints/secret error/Kyber256/v.txt", 'r') as f:
        lines_V = [next(f) for _ in range(nb_of_hints)]
    V = np.loadtxt(lines_V)

    with open("Data/Perfect Hints/secret error/Kyber256/l.txt", 'r') as g:
...
if __name__ == "__main__":
    with open("Data/Perfect Hints/secret error/Kyber256/es.txt", 'r') as g:
    ...
    for m in tqdm(range(200, 201, 10)):
        num_hint.append(m)
